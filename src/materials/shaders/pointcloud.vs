...
// GLSL ES 3.00 (WebGL2) only lets a sampler array be indexed by a constant
// integral expression, so the shadow loop can't do `uShadowMap[i]`. Dispatch
// on literal indices instead — an explicit unroll. Branches past
// `num_shadowmaps` are dropped by the preprocessor, so only samplers that were
// actually declared are referenced. The renderer currently binds a single
// shadow map (EDLRenderer); the extra cases keep this correct if that grows.
vec4 sampleShadowMap(int index, vec2 uv){
	#if num_shadowmaps > 0
	if(index == 0){ return texture(uShadowMap[0], uv); }
	#endif
	#if num_shadowmaps > 1
	if(index == 1){ return texture(uShadowMap[1], uv); }
	#endif
	#if num_shadowmaps > 2
	if(index == 2){ return texture(uShadowMap[2], uv); }
	#endif
	#if num_shadowmaps > 3
	if(index == 3){ return texture(uShadowMap[3], uv); }
	#endif
	return vec4(0.0);
}
#endif
...
float potreeRound(float number){
...
float getLOD(){
	...
	for(float i = 0.0; i <= 30.0; i++){
		...
		int index = int(potreeRound(4.0 * index3d.x + 2.0 * index3d.y + index3d.z));

		vec4 value = texture(visibleNodes, vec2(float(iOffset) / 2048.0, 0.0));
		int mask = int(potreeRound(value.r * 255.0));

		if(isBitSet(mask, index)){
			// there are more visible child nodes at this position
			int advanceG = int(potreeRound(value.g * 255.0)) * 256;
			int advanceB = int(potreeRound(value.b * 255.0));
...
void main() {
	...
	doClipping();
		...
		for(int i = 0; i < num_shadowmaps; i++){
			...
			for(int j = 0; j < 9; j++){
				vec4 depthMapValue = sampleShadowMap(i, vec2(u, v) + sampleLocations[j]);
			...
			if(u < 0.0 || u > 1.0 || v < 0.0 || v > 1.0 || nc.x < -1.0 || nc.x > 1.0 || nc.y < -1.0 || nc.y > 1.0 || nc.z < -1.0 || nc.z > 1.0){
				// outside the shadow-map frustum: leave color unchanged
			}else{
